...
def store_digits(n):
    ...
    while n1 // 10:
        n1 //= 10
        cnt *= 10
    return Link(n1, store_digits(n % cnt))
...
def two_list(vals, counts):
    ...
    """
    "*** YOUR CODE HERE ***"
    def f(vals, counts, cnt_val):
        if counts[cnt_val] == 0:
            cnt_val += 1
        counts[cnt_val] -= 1
        if counts[cnt_val] == 0 and cnt_val == len(vals) - 1:
            return Link(vals[cnt_val])
        return Link(vals[cnt_val], f(vals, counts, cnt_val))
