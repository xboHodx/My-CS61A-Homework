...
from fontTools.mtiLib import parseAlternate
...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps = [s for s in paragraphs if select(s)]
    if k >= len(ps):
        return ''
    else:
        return ps[k]
...
def accuracy(typed, source):
    ...
    corrent_num = 0
    len_min = min(typed_len, source_len)
    for i in range(len_min):
        if typed_words[i] == source_words[i]:
            corrent_num += 1
    return corrent_num / len(typed_words) * 100
    # END PROBLEM 3
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    len_ = min(len(source), len(typed))
    def f(index, cnt_):
        if index >= len_ or cnt_ > limit:
            return cnt_
        if typed[index] == source[index]:
            return f(index + 1, cnt_)
        else:
            return f(index + 1, cnt_ + 1)
    cnt = f(0, 0)
    cnt += abs(len(source) - len(typed))
    return cnt if cnt <= limit else limit + 1
    # END PROBLEM 6
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    match__ = [[] for _ in player_indices]
    for word in word_indices:
        fastest_player = 0
        for player in player_indices:
            if times[player][word] < times[fastest_player][word]:
                fastest_player = player
        match__[fastest_player].append(words[word])
    return match__
    # END PROBLEM 10
